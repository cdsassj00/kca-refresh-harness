"""engine.runner / stages / jobs 테스트 — MockLLM 으로 classify→chains→delta→impact→verify→brief→blind→compare→report→critic
흐름, 스키마 오류 재요청, 재개 건너뜀, 블라인드 격리, 강제 재실행, JobStore 를 확인한다. 네트워크 없음."""
...
def full_responses():
    """classify→chains→delta(도구 1회)→impact→verify→brief→blind→compare→(report 파이썬)→critic 순서의 응답."""
    return [
        ...
        final({"L1/verdicts.json": [VERDICT], "L2/traced/traced_conclusions.json": TRACED}),
...
def test_stages_for_layers_and_options():
    ...
    assert names == ["intake", "classify", "chains", "delta", "impact", "verify", "brief", "blind", "compare", "report", "critic"]
...
def test_full_pipeline_flow(make_report, settings, stub_search, core_dir):
    ...
    assert st == {"intake": "skipped", "classify": "done", "chains": "done", "delta": "done", "impact": "done", "verify": "done",
    ...
    assert llm.calls[0]["model"] == "mock/cheap" and llm.calls[6]["model"] == "mock/cheap" and llm.calls[1]["model"] == "mock/main"
    ...
    compare_user = llm.calls[8]["messages"][1]["content"]
    ...
    assert out["tokens"] == {"in": 1000, "out": 500} and out["cost_usd"] == pytest.approx(0.01)
