...
> **포털 경유와 헷갈리지 말 것 (2026-09-19 실호출로 정리).**
> 공공데이터포털의 「한국연구재단_KCI 논문정보서비스」(`B552540/KCIOpenApi/artiInfo`) 에는
> 상세기능이 여럿 있고, 그중 **`openApiD262List` 는 논문 서지를 준다**(ARTIID·PYEAR·TIK 제목한글·
> TIE 제목영문·SPAGE·EPAGE). D263·D264 는 저자, D267 은 초록이다.
> 처음에 `openApiD217List`(Full-Text 반입 논문 조회 — NUM·FTID·XMLFILE 같은 파일 관리 정보)만
> 보고 "포털에는 논문 서지가 없다"고 적었던 것은 **틀렸다.**
> 그러나 쓸 수 없는 이유는 따로 있다.
>   - **총 2,984건뿐이다**(KCI 전체가 아니라 일부 표본으로 보인다)
>   - **주제어로 거르는 변수가 없다.** `title`·`TIK`·`keyword`·`searchWord`·`pyear` 를 넣어도
>     결과가 똑같다(총건수·ARTIID 동일). 즉 검색이 아니라 통째로 내려받는 목록이다.
>   - 포털 인증키를 아래 KCI 직접 API 에 넣으면 `등록되지 않은 key 입니다` 가 온다. **다른 키다.**
> 따라서 "발간 이후 이 주제로 나온 논문" 을 찾으려면 아래 KCI 직접 API 가 필요하다.
